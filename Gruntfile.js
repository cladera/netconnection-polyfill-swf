module.exports = function (grunt) {
    ...
    mxmlc: {
      options: {
        ...
        metadata: {
          ...
          title: 'NetConnection Polyfill SWF',
          // `-description "http://www.adobe.com/flex"`
          description: '',
          // `-publisher "The Publisher"`
          publisher: 'Carles Galan Cladera <cgcladera@gmail.com>',
          // `-creator "The Author"`
          creator: 'Carles Galan Cladera',
          // `-language=EN`
          // `-language+=klingon`
          language: 'EN',
      ...
      netconnection_polyfill_swf: {
        files: {
          'dist/netconnection-polyfill.swf': ['src/NetConnectionPolyfill.as']
        }
      }
    },
  });
  ...
  grunt.registerMultiTask('mxmlc', 'Compiling SWF', function () {
    ...
    workerFn = function(f, callback) {
      ...
      //External libraries
      /*grunt.verbose.writeln('Libraries: %j', options.libraries);
      if(options.libraries) {
        grunt.verbose.writeln('Reading external libraries');
        for(var i = 0; i < options.libraries.length; i++) {
          var path = options.libraries[i];
          grunt.verbose.writeln('Getting external libraries form %s', path);
          var files = glob.sync(path);
          for(var j = 0; j < files.length; j++) {
            var lib = files[j];
            cmdLineOpts.push('-library-path+='+lib);
          }
        }
      }*/
      cmdLineOpts.push('--');
